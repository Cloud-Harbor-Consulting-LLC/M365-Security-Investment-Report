function ConvertTo-CHSIHtml {
    ...
    .DESCRIPTION
        Everything is inlined: stylesheet, fonts, logo, favicon and charts. The finished
    ...
    $favicon = Get-CHSIEmbeddedAsset -RelativePath 'favicon.svg'
    $logo    = Get-CHSILogoMarkup
    ...
    $body.Add((Get-CHSIHtmlMasthead -Report $Report -Logo $logo))
...
function Get-CHSIHtmlMasthead {
    [CmdletBinding()]
    [OutputType([string])]
    param([Parameter(Mandatory)][object]$Report, [string]$Logo)
...
<header class="ch-masthead">
  <div class="ch-masthead-inner">
    ...
    <div class="ch-masthead-meta">
      ...
      <div>Prepared by <strong>$(ConvertTo-CHSIHtmlEncoded $Report.Config.report.preparedBy)</strong></div>
...
$($collectorRows -join "`n")
  ...
  <dl class="ch-defs">
    ...
    <dd>The fraction of a SKU's price this tool attributes to its security capabilities, used from milestone M3 to dollarize unconfigured features. It is a Cloud Harbor allocation model, not a Microsoft-published figure, and is overridable per SKU.</dd>
    <dt>Expected loss</dt>
...
<footer class="ch-footer">
  ...
  <p>&copy; $((Get-Date).Year) Cloud Harbor Consulting LLC. Distributed under the MIT License.</p>
</footer>
"@
}
