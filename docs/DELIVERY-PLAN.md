...
This is a change of **medium**, and it changes the product's centre of gravity:
...
The consequence that matters most: **the override stops being a config feature and becomes a meeting feature.** When the CFO says *"we don't pay list, we pay $28"*, you type 28 and the idle-spend figure recalculates in front of them. That moment is the product. Nothing in a static file can do it.

### What survives from M0/M1

Roughly 60% of the code, and all of the hard-won correctness:

- The Graph collection layer and the GET-only chokepoint. Unchanged, and still the trust story.
- `sku-catalog.json`, `pricelist.json`, `feature-map.json`, `risk-model.json`. Unchanged, and still the IP.
...
The app is a **static site on GitHub Pages**. There is no backend, and there is nowhere for customer data to go, which is a stronger privacy position than any hosted SaaS could offer.
...
1. **Inline editing.** Every dollar figure is click-to-edit at the point it appears. An editable number keeps the room's attention where a settings page would lose it. Changing one price re-renders every dependent figure with a brief highlight so the room *sees* what moved.

2. **Provenance on every number.** Click any figure for its derivation, the endpoint it came from, and whether it is measured, allocated, or assumed. This is the natural extension of the discipline already in the code, which is never to show `$0` where the truth is "we could not look". It is what makes a CFO stop treating the report as a black box.

3. **Scenario modelling.** Toggle undeployed controls on and watch spend-realized and expected-loss move. This turns the roadmap from a list into a planning instrument, and it is the same arithmetic the report already does, run forward.
...
**Which roles can consent.** Because this tool uses **delegated** permissions only, the bar is lower than Global Administrator. **Cloud Application Administrator**, **Application Administrator** and **AI Administrator** can grant tenant-wide admin consent here, and so can **Privileged Role Administrator**. **Global Administrator** works as a superset of all of them. The documented restriction on the last two, that they cannot consent to Microsoft Graph *app roles*, applies to application permissions, which this tool never requests. Worth stating in the docs, because "you need Global Admin" is the assumption that stalls these conversations.
...
- **No analytics, no telemetry, no error reporting endpoint.** A security tool must not phone home.
...
`Snapshot` and `ReferenceData` are already close to this shape, and the M1 schemas mostly stand.
...
| **M9** | **Public-ready**: app-registration walkthrough, accessibility pass, docs, screenshots, licence and privacy review | Someone who has never met us can run it against their own tenant |
...
| "Stateless in v1.0" | **Relaxed deliberately.** Session files are local, user-owned, and never auto-read, on the same justification as snapshots. They are also the foundation for v1.1 delta tracking. |
...
| "No multi-tenant / MSP mode" | **Kept.** One tenant per session, no cross-tenant storage, nothing persisted server-side, because there is no server. An optional Cloud Harbor multi-tenant *app registration* (§5.1) is a consent convenience, not MSP mode. |
...
So the client ID is a build-time constant (`VITE_MSAL_CLIENT_ID`), and supplying your own is an option for organisations that will not accept a third-party app in their directory. It is never a prerequisite.

### Incremental consent

**Superseded during M4. All 5 scopes are now requested at sign-in.**

The original plan asked for `AuditLog.Read.All` and `SecurityEvents.Read.All` only when a section needed them, on the assumption that requesting them up front would make sign-in fail in tenants that cannot grant them. That assumption was wrong. Both are ordinary delegated permissions any tenant can consent to. The entitlement gate bites when the API is called, as a 403 the collectors already degrade around, and not at consent.

So one consent screen now buys a complete collection. If an administrator declines the full set, the app retries with the 3 required scopes, which keeps the licence inventory and spend analysis.

The rule underneath is unchanged: **an optional signal degrades a section, it never fails the run.** See `app/src/graph/scopes.ts`.
